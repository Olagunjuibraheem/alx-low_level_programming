...
{
	char *concat_nstr;
	unsigned int len1, len2, len12, len1n, i, j;
	...
	{
		len12 = len1 + len2;
		concat_nstr = malloc(sizeof(char) * (len12 + 1));
	}
	else
	{
		len1n = len1 + n;
		concat_nstr = malloc(sizeof(char) * (len1n + 1));
	...
	i = len1;
	j = 0;
	while (n < len2 && i < len1n)
		concat_nstr[i] = s2[j], i++, j++;
	while (n >= len2 && i < len12)
		concat_nstr[i] = s2[j], i++, j++;
	concat_nstr[i] = '\0';
	return (concat_nstr);
}
